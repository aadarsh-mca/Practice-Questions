...
public class UserDaoImpl implements UserDao {
	...
	public UserDaoImpl() throws SQLException {
		...
		// create pst1
		pst1 = connection.prepareStatement("select * from voting_users where email=? and password=?");
		/*
		 * id | first_name | last_name | email | password | dob | status | role
		 */
		pst2 = connection.prepareStatement("insert into voting_users values(default,?,?,?,?,?,?,?)");
		pst3 = connection.prepareStatement("update voting_users set password=? where email=? and password=?");
		pst4=connection.prepareStatement("delete from voting_users where id=?");
		pst5=connection.prepareStatement("update voting_users set status=? where id=?");
		System.out.println("user dao created !");
	}

	@Override
	public User authenticateUser(String email, String pwd) throws SQLException {
		System.out.println("authenticate user Query Executed !!!");
