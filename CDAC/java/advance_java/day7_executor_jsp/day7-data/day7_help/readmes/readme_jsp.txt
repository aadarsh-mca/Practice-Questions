...
8. Then it will call follwing method using thread created per clnt request : 
...
When _jspService rets , thread's run method is over & thread rets to the pool, where it can be used for servicing some other or same clnt's req.
...
eg - index.jsp --> index_jsp.java (public final class) --> index_jsp.class => translated page class
--> class loading --> instance : page (used rarely in the code !)
...
More concise alternative to JSP Expressions(for accessing attributes) : EL syntax (Expression Language : avlble from JSP 1.2 onwards)
...
Typically read only
-  through  methods-- inherently thread safe.

attributes -- page|request|session|application scoped objects
=> name : String , value : Object
Typically ctx scoped attributes -- inherently thread UN SAFE(since a single copy is shared across multiple clients , represented by different threads)
- must be accessed in a synchronized manner, using synchronized block.

These EL implicit objects are accessible ONLY via EL syntax.
...
eg : Suppose you have added ctx scoped attr --- loan_scheme
How in a Servlet ?
getServletContext().setAttribute("loan_scheme", loanScheme);
How in a JSP ?
<%
 application.setAttribute("loan_scheme", loanScheme);
%>
How to retrieve it's value in JSP ?
Options
1. Scriptlet
2. JSP Expresssion
3. EL syntax
...
3.3 JSP declarations (private members of the translated page class)
...
1. import="comma separated list of packaged clasess|interfaces"
2. session="true|false"  boolean attribute. default=true(indicating WC supplies HttpSession object to all JSPs)
To disable session tracking, spectify session="false"

3. Centralized error handling in JSP
...
-- evaluates to pageContext.getException().getMessage()
OR
...
ERROR Mesg :  ${pageContext.exception.message} <br/>
ERROR causing Page :  ${pageContext.errorData.requestURI}<br/>
...
true=>informing WC--- JSP is already written in thread -safe manner ---- DON'T apply thread safety.




false=>informing WC ---to apply thread safety.
...
eg ---Context scoped attrs are inherently thread -un safe. So access them always from within synched block.
eg : 
<%
...
Servlet class can imple. tag i/f(marker i/f) -- javax.servlet.SingleThreadModel(DEPRECATED) -Marker interface
 WC ensures only 1thread (representing client request) can invoke service method. --NOT  recommended.
...
4.3 <jsp:param> 
It's a  nested tag. 
To be nested , within <jsp:forward> | <jsp:include>
Syntax - 
