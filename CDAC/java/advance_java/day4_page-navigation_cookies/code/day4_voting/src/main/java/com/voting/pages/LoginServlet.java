...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			// null checking
			if (user != null) {
				// authentication successful , now proceed to
				// role based authorization
//				pw.print("<h5>from login page -  Login Success </h5>"); - contents are ignored | cleared !!!!!
//				pw.flush(); - un comment to understand - IllegalStateException
//				pw.print("<h6> Hello , " + user.getFirstName() + " " + user.getLastName() + "</h6>");
				// Cookie steps - 1 create a cookie
				...
				if (user.getUserRole().equals("admin")) {
					...
					//Resp Packet - SC 302 , 
					//Resp headers - Location - list , Set-Cookie : user_details : to string of user
					// body - empty
				}
